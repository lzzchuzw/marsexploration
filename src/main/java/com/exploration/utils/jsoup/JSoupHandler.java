package com.exploration.utils.jsoup;
...
public class JSoupHandler {
	...
	public static Map<String,Object> parserResponseRet_visitJDLoginHomePage(final ResponseRet responseRet){
		...
		if(null!=doc) {
			...
			//登录二维码
			Element qrCodeImgDiv = doc.getElementsByClass("qrcode-img").first();
			Element qrCodeImg = qrCodeImgDiv.getElementsByTag("img").first();
			String qrCodeImgSrc = qrCodeImg.attr("src");
			String qrCodeImgUrl = "https:"+qrCodeImgSrc;
			System.out.println("qrCodeImgUrl = "+qrCodeImgUrl);
			visitJDLoginHomePageMap.put("authCodeUrl", authCodeUrl);
			visitJDLoginHomePageMap.put("qrCodeImgUrl", qrCodeImgUrl);
		}
		return visitJDLoginHomePageMap;
	}
	/**
	 * 解析页面登录页面获取扫码登录的二维码url
	 * @param responseRet
	 * @return
	 */
	public static String parserQrCodeUrl_visitJDLoginHomePage(final ResponseRet responseRet){
		...
		if(null!=doc) {
			...
			Element qrCodeImg = qrCodeImgDiv.getElementsByTag("img").first();
			String qrCodeImgSrc = qrCodeImg.attr("src");
			qrCodeImgUrl = "https:"+qrCodeImgSrc;
			System.out.println("qrCodeImgUrl = "+qrCodeImgUrl);
		}
		return qrCodeImgUrl;
	}
	...
	public static List<String> parseOrderIdListFromOrderListPage(final ResponseRet responseRet){
		List<String> orderStringList = null;
		if(null==responseRet || null==responseRet.getRetContent()) {
		...
		if(null!=doc) {
			Elements orderIdLinks = doc.getElementsByAttributeValue("name", "orderIdLinks");
			for(Element e:orderIdLinks) {
				String orderId = e.val();
				orderStringList.add(orderId);
			}
		}
		return orderStringList;
	}
