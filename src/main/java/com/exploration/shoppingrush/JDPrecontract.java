 ...
 * @Description 京东预约
 * @author lzz
 * @time 2018年2月9日 上午10:25:52
 */
public class JDPrecontract {
	...
	public Map<String,Object> fetchPrecontractShoppingRushUrl(HttpClientRequestHandler requestHandler, String productId) {
		...
		String url = "https://yushou.jd.com/youshouinfo.action?callback=fetchJSON&sku="+productId;
		...
		RequestBuilder requestBuilder = RequestBuilder.get()// Get 方法
				                                      .setUri(url);// 设置访问的Uri
		System.out.println("获取预抢购商品的预约Url = "+url);
		// 设置访问的Header
	 ...
	 * @param
	 * @return 是否预约成功
	 * precontractUrl  https://yushou.jd.com/toYuyue.action?sku=6138263&key=c422b1c98126552e8b94dfd53c846bce
	 */
	public boolean precontractShoppingRush(HttpClientRequestHandler requestHandler,Map<String,Object>  fetchPrecontractShoppingRushUrlMap) {
		...
		// 设置访问的Header
		HttpRequestHeaderGenerator.setPrecontractShoppingRushHeaders(requestBuilder,skuId);
		// 生成访问方法
		HttpUriRequest requestMethod = requestBuilder.build();
		// 保存访问方法
		requestHandler.setRequestMethod(requestMethod);		
